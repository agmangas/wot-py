...
def test_regenerate_links():
    """Interaction Links are properly generated by the WebSockets server."""
    ...
    exposed_thing.add_event(event_init=event_init_01)

    def _assert_websockets_links():
        for intrction in exposed_thing.thing.interaction:
            assert len(intrction.link) == 1
            for lnk in intrction.link:
                assert lnk.protocol == Protocols.WEBSOCKETS

    websockets_server = WebsocketServer()
    websockets_server.add_exposed_thing(exposed_thing)
