...
class MQTTHandlerRunner(object):
    ...
    def _connect(self):
        ...
        self._log(logging.DEBUG, "Using MQTT client config: {}".format(self._client_config))

        hbmqtt_client = MQTTClient(config=self._client_config)

        self._log(logging.DEBUG, "Connecting MQTT client to broker: {}".format(self._broker_url))
    ...
    def _disconnect(self):
        ...
        try:
            ...
            self._log(logging.WARNING, "Error disconnecting MQTT client: {}".format(ex), exc_info=True)
        finally:
            self._client = None
